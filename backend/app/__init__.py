...
server = Flask(__name__, static_folder=os.path.join(currentDir, "app", "static"))
CORS(server)
...
def joint_threads():
    ...
    JointWav.joint_audio(files, os.path.join(currentDir, "app", "audio", filename))
...
@server.route("/render_voice", methods=["POST"])  #追加
def render():
...
@server.route("/glue", methods=["POST"])
def glue():
    dt_now = datetime.datetime.now().strftime("%Y%m%d%H%M%S%f")
...
@server.route("/auto_onomatope", methods=["POST"])
def auto_onomatope():
    ...
    return_data = {"fileUrl": os.path.join(currentDir, "app", "audio", filename)}
    return jsonify(return_data)
